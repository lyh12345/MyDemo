...
import com.taobao.rigel.rap.reqforward.action.ReqForwardAction;
import com.taobao.rigel.rap.utils.XmlUtils;
...
public class ProjectDaoImpl extends HibernateDaoSupport implements ProjectDao {
    ...
    public String updateProject(int id, String projectData,
    	...
    	System.out.println("json is have :"+projectData);
    	Session session = currentSession();
        ...
        for (Module module : projectClient.getModuleList()) {
            ...
            for (Page page : module.getPageList()) {
                ...
                //	System.out.println("updatre :"+action.getId());
                    ...
                    if (actionServer == null) {
                        ...
                        /*String path = "/com/taobao/rigel/rap/reqforward/action/struts.xml";
                        
							String real = ServletActionContext.getServletContext().getRealPath(path);
							String filePath = ReqForwardAction.class.getClassLoader().getResource("/com/taobao/rigel/rap/reqforward/action/struts.xml").getPath();
							filePath = filePath.substring(1, filePath.length());
                        System.out.println("接口的路径"+real);
                        System.out.println("接口的路径2"+filePath);
                      //  String filePath ="D:/MyDownloads/Download/apache-tomcat-7.0.91/webapps/RAP-master_internet/WEB-INF/classes/com/taobao/rigel/rap/reqforward/action/struts.xml";
                        System.out.println("接口"+action.getRequestUrl());
                        Document doc = XmlUtils.getDocument(new File(filePath));
                        
                        if(!XmlUtils.ishasPackage(action.getRequestUrl(), doc))
                        {
                        	XmlUtils.addElement(doc, action.getRequestUrl());
                        	XmlUtils.savXml(doc, filePath);
                        	
                        	SAXReader saxReader = new SAXReader();
                        	try {
								org.dom4j.Document document4 = saxReader.read(new File(filePath));
								document4.addDocType("struts","-//Apache Software Foundation//DTD Struts Configuration 2.0//EN\" \"classpath:struts-2.3.dtd", null);
								OutputFormat format = OutputFormat.createPrettyPrint();
								XMLWriter writer = new XMLWriter(new FileOutputStream(filePath),format);
								writer.write(document4);
								writer.close();
							} catch (Exception e) {
                ...
                //       System.out.println("parameterServer :"+parameterServer.toString());
                        parameterServer.update(parameter);
                //        System.out.println("parameterServer :"+parameterServer.toString());
                        for (Parameter childParameter : parameter
                                .getParameterList()) {
    ...
    //修改private为public
    public List<Action> getActionListOfProject(int projectId) {
