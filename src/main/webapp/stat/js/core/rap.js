...
//        var len = arr.length;
//        arr[len] = b.g("editAFloater-pageId").value;
        console.log(arr);
//        var pageList = p.getPageList(_curModuleId);
        var pid = b.g("editAFloater-pageId").value;
        ...
        for(var i=0;arr != null && i<arr.length;i++)
        	...
        	if(arr[i] != pid)
        		...
        		action.pageId = p.getPage(arr[i]).id;
                action.name = b.g("editAFloater-name").value;
                action.requestType = getSelectedValue("editAFloater-type");
                action.requestUrl = b.g("editAFloater-requestUrl").value;
                action.responseTemplate = b.g("editAFloater-responseTemplate").value;
    ...
    ws.doAddP = function(ac) {
        ...
        //添加action
        for(var i=0;ac != null && i<ac.length;i++)
        	{
	        	action = p.getAction(ac[i]);
...
//	            console.log(action);
    ...
    ws.quickSave = function(commitMsg) {
        ...
        console.log("执行1");
        console.log("执行2");
        console.log(getProjectDataJson());
    ...
    ws.closeActionOpFloater = function(userConfirm) {
        if (userConfirm) {
            ...
            if (targetMID && fu!=null) {
            	for(var i=0;i < fu.length;i++)
            		{
            		actionOperate(targetMID, fu[i], opType);
            		}
